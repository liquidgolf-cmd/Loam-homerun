export default function Philosophy() {
  return (
    <section className="py-20 px-4 bg-loam-white">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-loam-text mb-4">
            The Philosophy
          </h2>
        </div>

        <div className="space-y-8">
          <div className="bg-loam-cream p-8 rounded-xl border-l-4 border-loam-brown-600">
            <h3 className="text-2xl font-bold text-loam-text mb-4">
              You Have the Answers
            </h3>
            <p className="text-lg text-loam-text/80 leading-relaxed">
              You started this business. You know it better than anyone. My role isn't to tell you what to do—it's to ask the right questions at the right time, helping you unlock the insights you already possess but can't quite see.
            </p>
          </div>

          <div className="bg-loam-sage/20 p-8 rounded-xl border-l-4 border-loam-teal">
            <h3 className="text-2xl font-bold text-loam-text mb-4">
              Question-Based Discovery
            </h3>
            <p className="text-lg text-loam-text/80 leading-relaxed">
              loam strategy helps small business owners move from confusion to clarity through guided conversations, AI-powered audits, and a simple framework you'll keep using long after we're done. Unlike traditional consultants who provide solutions, I guide you through a process of self-discovery. The Homerun Method gives you a framework to continue using long after our engagement ends, building capability rather than dependency.
            </p>
          </div>

          <div className="bg-loam-cream p-8 rounded-xl border-l-4 border-loam-brown-600">
            <h3 className="text-2xl font-bold text-loam-text mb-4">
              Like Loam Soil
            </h3>
            <p className="text-lg text-loam-text/80 leading-relaxed">
              Just as loam soil contains the perfect blend of elements to create fertile growing conditions, great businesses need the right balance of strategic elements. It's about creating a strong foundation—not forcing quick fixes.
            </p>
          </div>
        </div>
      </div>
    </section>
  )
}

